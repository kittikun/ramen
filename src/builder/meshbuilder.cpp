...
namespace ramen
{
    ...
    {
		FbxNode* node = m_pFbxMesh->findNode(FbxNodeAttribute::eMesh);
		...
		if (fbxMesh == nullptr) {
			LOGE << "Could not get fbxMesh from scene";
			return boost::shared_ptr<Mesh>();
		}
		...
		mesh->m_bHasNormal = fbxMesh->GetElementNormalCount() > 0;
		mesh->m_bHasUV = fbxMesh->GetElementUVCount() > 0;
		...
		if (mesh->m_bByControlPoint) {
			...
			if (mesh->m_bHasUV) {
				lUVElement = fbxMesh->GetElementUV(0);
			}
		...
		for (int polygonIndex = 0; polygonIndex < polygonCount; ++polygonIndex)	{
			...
			for (int verticeIndex = 0; verticeIndex < 3; ++verticeIndex)	{
				...
				if (mesh->m_bByControlPoint)	{
					...
					if (mesh->m_bHasNormal)	{
						fbxMesh->GetPolygonVertexNormal(polygonIndex, verticeIndex, lCurrentNormal);
					...
					if (mesh->m_bHasUV)	{
						bool lUnmappedUV;
						fbxMesh->GetPolygonVertexUV(polygonIndex, verticeIndex, lUVName, lCurrentUV, lUnmappedUV);
