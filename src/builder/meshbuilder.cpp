...
namespace ramen
{
    ...
    void MeshBuilder::resizeArrays(boost::shared_ptr<Mesh>& mesh)
    {
        ...
        mesh->m_vertices.resize(mesh->m_iPolygonVertexCount * Mesh::StrideVertex);
        mesh->m_indices.resize(mesh->m_iPolygonCount * 3);

        if (mesh->m_bHasNormal)	{
            mesh->m_normals.resize(mesh->m_iPolygonVertexCount * Mesh::StrideNormal);
        }

        m_pFbxMesh->GetUVSetNames(uvNames);

        if (mesh->m_bHasUV && uvNames.GetCount()) {
            mesh->m_UVs.resize(mesh->m_iPolygonVertexCount * Mesh::StrideUV);
    ...
    void MeshBuilder::process()
    {
        ...
        if (mesh->m_bByControlPoint) {
            ...
            for (uint32_t index = 0; index < mesh->m_iPolygonVertexCount; ++index) {
                ...
                mesh->m_vertices[index * Mesh::StrideVertex] = static_cast<float>(currentVertex[0]);
                mesh->m_vertices[index * Mesh::StrideVertex + 1] = static_cast<float>(currentVertex[1]);
                mesh->m_vertices[index * Mesh::StrideVertex + 2] = static_cast<float>(currentVertex[2]);
                mesh->m_vertices[index * Mesh::StrideVertex + 3] = 1;
                ...
                if (mesh->m_bHasNormal)	{
                    ...
                    mesh->m_normals[index * Mesh::StrideNormal] = static_cast<float>(currentNormal[0]);
                    mesh->m_normals[index * Mesh::StrideNormal + 1] = static_cast<float>(currentNormal[1]);
                    mesh->m_normals[index * Mesh::StrideNormal + 2] = static_cast<float>(currentNormal[2]);
                }
                ...
                if (mesh->m_bHasUV) {
                    ...
                    mesh->m_UVs[index * Mesh::StrideUV] = static_cast<float>(currentUV[0]);
                    mesh->m_UVs[index * Mesh::StrideUV + 1] = static_cast<float>(currentUV[1]);
                }
            }
        }
        ...
        for (int32_t polygonIndex = 0; polygonIndex < mesh->m_iPolygonCount; ++polygonIndex)	{
            ...
            for (int verticeIndex = 0; verticeIndex < (Mesh::StrideVertex - 1); ++verticeIndex)	{
                ...
                if (mesh->m_bByControlPoint)	{
                    ...
                    mesh->m_vertices[vertexCount * Mesh::StrideVertex] = static_cast<float>(currentVertex[0]);
                    mesh->m_vertices[vertexCount * Mesh::StrideVertex + 1] = static_cast<float>(currentVertex[1]);
                    mesh->m_vertices[vertexCount * Mesh::StrideVertex + 2] = static_cast<float>(currentVertex[2]);
                    mesh->m_vertices[vertexCount * Mesh::StrideVertex + 3] = 1;

                    if (mesh->m_bHasNormal)	{
                        m_pFbxMesh->GetPolygonVertexNormal(polygonIndex, verticeIndex, currentNormal);
                        mesh->m_normals[vertexCount * Mesh::StrideNormal] = static_cast<float>(currentNormal[0]);
                        mesh->m_normals[vertexCount * Mesh::StrideNormal + 1] = static_cast<float>(currentNormal[1]);
                        mesh->m_normals[vertexCount * Mesh::StrideNormal + 2] = static_cast<float>(currentNormal[2]);
                    }

                    if (mesh->m_bHasUV)	{
                        bool lUnmappedUV;
                        m_pFbxMesh->GetPolygonVertexUV(polygonIndex, verticeIndex, m_strUVName, currentUV, lUnmappedUV);
                        mesh->m_UVs[vertexCount * Mesh::StrideUV] = static_cast<float>(currentUV[0]);
                        mesh->m_UVs[vertexCount * Mesh::StrideUV + 1] = static_cast<float>(currentUV[1]);
                    }
                }
                ++vertexCount;
            }
            ++mesh->m_subMeshes[materiaindex].triangleCount;
        }
